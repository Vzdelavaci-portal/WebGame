...
const ui={gold:el('gold'),hp:el('castleHp'),wave:el('wave'),kills:el('kills'),score:el('score'),best:el('best'),map:el('mapName'),overlay:el('overlay'),mine:el('costMine'),walls:el('costWalls'),smith:el('costBlacksmith')};
...
let goldIncome=1,damageBonus=0,wallLvl=0,mineLvl=0,smithLvl=0,unitLevels=createUnitLevels(),recruitedTypes=[],currentMap='greenValley',currentDifficulty='normal',selectedMap='greenValley',selectedDifficulty='normal',profile=GameStorage.loadProfile(),hazardTimer=0,running=false,paused=false,anim,spawnTimer=0,spawned=0,toSpawn=0,waveActive=false,waveDelay=140,goldTimer=0,shake=0,frame=0;
...
function startGame(mapId=selectedMap,difficultyId=selectedDifficulty){if(!MAP_CONFIG[mapId]||!profile.unlockedMaps.includes(mapId))mapId='greenValley';if(!DIFFICULTY_CONFIG[difficultyId])difficultyId='normal';GameStorage.clearGame();currentMap=mapId;currentDifficulty=difficultyId;units=[];enemies=[];projectiles=[];particles=[];texts=[];recruitedTypes=[];gold=GAME_CONFIG.startingGold;castleHp=GAME_CONFIG.startingCastleHp;castleMaxHp=GAME_CONFIG.startingCastleHp;wave=GAME_CONFIG.startingWave;kills=0;score=0;goldIncome=1;damageBonus=0;wallLvl=0;mineLvl=0;smithLvl=0;unitLevels=createUnitLevels();spawnTimer=0;spawned=0;toSpawn=countWave();waveActive=true;waveDelay=140;goldTimer=0;hazardTimer=0;shake=0;frame=0;running=true;paused=false;ui.overlay.style.display='none';updateUI();cancelAnimationFrame(anim);loop()}
function getSaveState(){return{gold,castleHp,castleMaxHp,wave,kills,score,goldIncome,damageBonus,wallLvl,mineLvl,smithLvl,unitLevels,recruitedTypes,currentMap,currentDifficulty,hazardTimer,spawnTimer,spawned,toSpawn,waveActive,waveDelay,goldTimer,frame,units:units.filter(u=>!u.dead&&u.hp>0).map(u=>({...u})),enemies:enemies.filter(e=>!e.dead&&e.hp>0).map(e=>({...e}))}}
...
function showStartMenu(){profile=GameStorage.loadProfile();let canContinue=GameStorage.hasGame();ui.overlay.innerHTML=`<div class="panel"><h2>🏰 Brave Fortress V5</h2><p>Defeat regional bosses, unlock achievements and build your campaign profile.</p><div class="menu-actions">${canContinue?'<button id="continueBtn">Continue</button>':''}<button id="newGameBtn">New Game</button><button id="profileBtn">Profile</button></div></div>`;ui.overlay.style.display='grid';if(canContinue)el('continueBtn').onclick=continueGame;el('newGameBtn').onclick=showNewGameMenu;el('profileBtn').onclick=showProfile}
function showNewGameMenu(){selectedMap=profile.unlockedMaps.includes(selectedMap)?selectedMap:'greenValley';let maps=Object.entries(MAP_CONFIG).map(([id,map])=>{let unlocked=profile.unlockedMaps.includes(id);return `<button class="option-card${id===selectedMap?' selected':''}" data-map="${id}" ${unlocked?'':'disabled'}><strong>${map.icon} ${map.name}</strong><span>${unlocked?map.description:'Locked — defeat the boss of the previous region.'}</span></button>`}).join('');let difficulties=Object.entries(DIFFICULTY_CONFIG).map(([id,difficulty])=>`<button class="option-card${id===selectedDifficulty?' selected':''}" data-difficulty="${id}"><strong>${difficulty.name}</strong><span>${id==='easy'?'Weaker enemies':id==='hard'?'Stronger enemies, better rewards':'Standard balance'}</span></button>`).join('');ui.overlay.innerHTML=`<div class="panel"><h2>New Campaign</h2><div class="new-game-options"><div class="option-group"><strong>Region</strong><div class="option-grid">${maps}</div></div><div class="option-group"><strong>Difficulty</strong><div class="option-grid">${difficulties}</div></div></div><div class="menu-actions"><button id="backBtn">Back</button><button id="launchBtn">Start Campaign</button></div></div>`;ui.overlay.querySelectorAll('[data-map]').forEach(button=>button.onclick=()=>{selectedMap=button.dataset.map;showNewGameMenu()});ui.overlay.querySelectorAll('[data-difficulty]').forEach(button=>button.onclick=()=>{selectedDifficulty=button.dataset.difficulty;showNewGameMenu()});el('backBtn').onclick=showStartMenu;el('launchBtn').onclick=()=>startGame(selectedMap,selectedDifficulty)}
function loop(){if(!running)return;if(!paused){update();draw()}anim=requestAnimationFrame(loop)}
function update(){frame++;income();mapHazards();waves();updateUnits();updateEnemies();updateProjectiles();updParticles();updTexts();updateUI();if(shake>0)shake--}
function mineCost(){return 50+mineLvl*35}function wallsCost(){return 60+wallLvl*45}function smithCost(){return 70+smithLvl*50}
function income(){if(++goldTimer>=60){gold+=goldIncome*MAP_CONFIG[currentMap].effects.incomeMultiplier;goldTimer=0}}
function mapHazards(){let damage=MAP_CONFIG[currentMap].effects.hazardDamage;if(!damage)return;if(++hazardTimer>=600){hazardTimer=0;hitCastle(damage);text(W/2,95,'VOLCANIC ERUPTION','#fb923c')}}
function countWave(){return WaveDirector.getEnemyCount(wave)}
function waves(){if(!waveActive){if(--waveDelay<=0){wave++;toSpawn=countWave();spawned=0;waveActive=true;text(W/2,82,WaveDirector.getTitle(wave,currentMap),WaveDirector.getTitleColor(wave))}return}if(++spawnTimer>Math.max(34,95-wave*4)&&spawned<toSpawn){spawnForWave();spawned++;spawnTimer=0}if(spawned>=toSpawn&&enemies.length===0){handleWaveCleared()}}
function handleWaveCleared(){gold+=(35+wave*5)*DIFFICULTY_CONFIG[currentDifficulty].reward;text(W/2,82,`WAVE ${wave} CLEARED`,'#22c55e');unlockAchievement(wave>=5?'waveFive':null);if(wave%10===0){completeMap();return}waveActive=false;waveDelay=160;saveCurrentGame(false)}
function completeMap(){running=false;GameStorage.clearGame();profile.victories[currentMap]=true;profile.defeatedBosses[currentMap]=BOSS_CONFIG[currentMap].name;unlockAchievement(currentMap==='greenValley'?'greenVictor':currentMap==='frozenKingdom'?'frozenVictor':'volcanicVictor',false);if(currentDifficulty==='hard')unlockAchievement('hardWin',false);let next=currentMap==='greenValley'?'frozenKingdom':currentMap==='frozenKingdom'?'volcanicLands':null,unlockMsg='';if(next&&!profile.unlockedMaps.includes(next)){profile.unlockedMaps.push(next);unlockMsg=`<br>${MAP_CONFIG[next].icon} ${MAP_CONFIG[next].name} unlocked.`}GameStorage.saveProfile(profile);cancelAnimationFrame(anim);showOverlay('🏆 '+BOSS_CONFIG[currentMap].victoryTitle,`You defeated ${BOSS_CONFIG[currentMap].name} on ${DIFFICULTY_CONFIG[currentDifficulty].name}.<br>Score: ${score}.${unlockMsg}`,'Campaign Menu',showStartMenu)}
function spawnForWave(){spawnEnemy(WaveDirector.getEnemyType(wave,currentMap))}
function spawnEnemy(type,x=spawnX){let c=enemiesCfg[type],m=1+wave*.12,difficulty=DIFFICULTY_CONFIG[currentDifficulty],map=MAP_CONFIG[currentMap];let hp=Math.floor(c.hp*m*difficulty.enemyHp);enemies.push({type,x,y:c.flying?groundY-105:groundY,hp,maxHp:hp,damage:Math.floor(c.damage*(1+wave*.06)*difficulty.enemyDamage),range:c.range,speed:c.speed*map.effects.enemySpeedMultiplier,rate:c.rate,cool:Math.random()*30,reward:Math.round(c.reward*difficulty.reward),color:c.color,boss:!!c.boss,mapBoss:!!c.mapBoss,dragon:!!c.dragon,flying:!!c.flying,summons:!!c.summons,summonTimer:240,chill:!!c.chill,burnAura:!!c.burnAura})}
function spawnSkeleton(x){let c=enemiesCfg.skeleton,difficulty=DIFFICULTY_CONFIG[currentDifficulty],map=MAP_CONFIG[currentMap],hp=Math.round((c.hp+wave*3)*difficulty.enemyHp);enemies.push({type:'skeleton',x,y:groundY,hp,maxHp:hp,damage:Math.round(c.damage*difficulty.enemyDamage),range:c.range,speed:c.speed*map.effects.enemySpeedMultiplier,rate:c.rate,cool:20,reward:Math.round(c.reward*difficulty.reward),color:c.color})}
...
function upgradeUnitType(type){if(!running||paused)return;let cost=unitUpgradeCost(type);if(cost===null)return;if(gold<cost)return text(W/2,120,`Need ${cost} gold`,'#dc2626');gold-=cost;unitLevels[type]++;let level=unitLevels[type],stats=unitStats(type,level);units.filter(u=>u.type===type&&u.hp>0).forEach(u=>{let hpRatio=u.hp/u.maxHp;u.level=level;u.maxHp=stats.hp;u.hp=Math.max(1,Math.round(stats.hp*hpRatio));u.damage=stats.damage;u.rate=stats.rate;u.ability=level===3?UNIT_UPGRADE_CONFIG.units[type].ability:null});if(level===3)unlockAchievement('eliteArmy');text(W/2,120,`${unitsCfg[type].name} LEVEL ${level}`,'#fbbf24');updateUI();saveCurrentGame(false)}
function spawnUnit(type){if(!running||paused)return;let c=unitsCfg[type],level=unitLevels[type],stats=unitStats(type,level);if(gold<c.cost){text(170,120,'Not enough gold','#dc2626');return}gold-=c.cost;if(!recruitedTypes.includes(type)){recruitedTypes.push(type);if(recruitedTypes.length===Object.keys(unitsCfg).length)unlockAchievement('fullRoster')}units.push({type,level,x:castleX+55,y:groundY,hp:stats.hp,maxHp:stats.hp,damage:stats.damage,range:c.range,speed:c.speed*MAP_CONFIG[currentMap].effects.unitSpeedMultiplier,rate:stats.rate,cool:0,color:c.color,ranged:!!c.ranged,bolt:!!c.bolt,shield:!!c.shield,elite:!!c.elite,magic:!!c.magic,splash:c.splash||0,healer:!!c.healer,ability:level===3?UNIT_UPGRADE_CONFIG.units[type].ability:null});part(castleX+55,groundY,c.color,16)}
...
function updateEnemies(){enemies.forEach(e=>{if(e.burnTicks>0){e.burnTicks--;if(e.burnTicks%30===0){e.hp-=e.burnDamage;part(e.x,e.y-20,'#f97316',6)}}if(e.burnAura&&frame%90===0){units.filter(u=>Math.abs(u.x-e.x)<70).forEach(u=>{u.hp-=Math.max(2,Math.round(e.damage*.18));part(u.x,u.y-22,'#fb923c',8)})}if(e.hp<=0){e.dead=true;kills++;if(kills===1)unlockAchievement('firstBlood');gold+=e.reward;score+=e.boss?500:50;text(e.x,e.y-44,`+${e.reward} gold`,'#fbbf24');part(e.x,e.y,e.color,e.boss?50:22);return}if(e.summons&&--e.summonTimer<=0){spawnSkeleton(e.x+35);e.summonTimer=300;text(e.x,e.y-55,'SUMMON','#e5e7eb')}let t=nearestUnit(e);if(t&&Math.abs(t.x-e.x)<=e.range){if(--e.cool<=0){e.dragon?dragonFire(e):hitUnit(e,t);e.cool=e.rate}}else if(e.x<=castleX+45){if(--e.cool<=0){hitCastle(e.damage);e.cool=e.rate}}else e.x-=e.speed});enemies=enemies.filter(e=>!e.dead)}
...
function hitUnit(e,u){let damage=u.ability==='30% Block'?Math.ceil(e.damage*.7):e.damage;u.hp-=damage;if(e.chill){u.cool+=18;text(u.x,u.y-52,'CHILL','#67e8f9')}part(u.x,u.y-20,e.color,8)}
...
function bg(){let p=MAP_CONFIG[currentMap].palette,g=ctx.createLinearGradient(0,0,0,H);g.addColorStop(0,p.skyTop);g.addColorStop(.5,p.skyMiddle);g.addColorStop(1,p.skyBottom);ctx.fillStyle=g;ctx.fillRect(0,0,W,H);ctx.fillStyle=p.sun;ctx.beginPath();ctx.arc(760,90,46,0,Math.PI*2);ctx.fill();mountains();if(currentMap!=='volcanicLands'){cloud(170,95,1);cloud(510,82,.8)}if(currentMap==='greenValley'){tree(250,groundY+5,1);tree(875,groundY+5,.85)}else if(currentMap==='frozenKingdom'){snowPine(250,groundY+5,1);snowPine(875,groundY+5,.85)}else{lavaVent(700,groundY+20);lavaVent(590,groundY+18)}rock(690,groundY+35);rock(595,groundY+32);ctx.fillStyle=p.ground;ctx.fillRect(0,groundY+15,W,H-groundY);ctx.fillStyle=p.groundDark;ctx.fillRect(0,groundY+45,W,H-groundY);ctx.fillStyle=p.road;ctx.fillRect(0,groundY+27,W,18);ctx.strokeStyle='rgba(255,255,255,.16)';ctx.lineWidth=2;for(let x=0;x<W;x+=60){ctx.beginPath();ctx.moveTo(x,groundY+36);ctx.lineTo(x+36,groundY+36);ctx.stroke()}}
function mountains(){let p=MAP_CONFIG[currentMap].palette;ctx.fillStyle=p.mountainFar;poly([[-80,groundY+15],[160,150],[410,groundY+15]]);ctx.fillStyle=p.mountainNear;poly([[190,groundY+15],[430,115],[720,groundY+15]]);ctx.fillStyle=p.mountainFar;poly([[580,groundY+15],[790,170],[1030,groundY+15]]);ctx.fillStyle=currentMap==='volcanicLands'?'#f97316':'#e2e8f0';poly([[430,115],[392,174],[468,166]]);ctx.fillStyle=currentMap==='volcanicLands'?'#dc2626':'#cbd5e1';poly([[790,170],[760,220],[820,212]])}
...
function snowPine(x,y,s){ctx.fillStyle='#475569';ctx.fillRect(x-4*s,y-40*s,8*s,40*s);ctx.fillStyle='#e0f2fe';poly([[x,y-82*s],[x-28*s,y-28*s],[x+28*s,y-28*s]]);ctx.fillStyle='#bae6fd';poly([[x,y-62*s],[x-23*s,y-12*s],[x+23*s,y-12*s]])}
function lavaVent(x,y){ctx.fillStyle='#18181b';ctx.beginPath();ctx.ellipse(x,y,24,11,0,0,Math.PI*2);ctx.fill();ctx.fillStyle='#fb923c';ctx.beginPath();ctx.ellipse(x,y-2,13,5,0,0,Math.PI*2);ctx.fill()}
...
function drawEnemy(e){if(e.type==='goblin')goblin(e);if(e.type==='orc')orc(e);if(e.type==='troll')troll(e);if(e.type==='skeleton')skeleton(e);if(e.type==='necromancer')necromancer(e);if(e.type==='boss')ogre(e);if(e.type==='dragon')dragon(e);if(e.type==='bandit')bandit(e);if(e.type==='iceWraith')iceWraith(e);if(e.type==='lavaGolem')lavaGolem(e);if(e.type==='forestAncient')forestAncient(e);if(e.type==='frostGiant')frostGiant(e);if(e.type==='fireDemon')fireDemon(e);hp(e,e.color,e.boss?86:44,e.dragon?96:e.mapBoss?104:62)}
...
function bandit(e){human(e,'#92400e','#fed7aa','#451a03');ctx.strokeStyle='#d6d3d1';ctx.lineWidth=3;line(e.x+12,e.y-23,e.x+28,e.y-38)}
function iceWraith(e){ctx.fillStyle='rgba(103,232,249,.82)';ctx.strokeStyle='#0e7490';ctx.lineWidth=2;ctx.beginPath();ctx.arc(e.x,e.y-38,16,0,Math.PI*2);ctx.fill();ctx.stroke();ctx.beginPath();ctx.moveTo(e.x-16,e.y-25);ctx.quadraticCurveTo(e.x,e.y+12,e.x+16,e.y-25);ctx.fill();ctx.stroke();ctx.strokeStyle='#cffafe';line(e.x-20,e.y-35,e.x-34,e.y-50)}
function lavaGolem(e){ctx.fillStyle='#3f3f46';ctx.strokeStyle='#fb923c';ctx.lineWidth=3;rr(e.x-22,e.y-58,44,54,10);ctx.fill();ctx.stroke();legs(e,'#27272a',6,-5,18);ctx.fillStyle='#fb923c';ctx.beginPath();ctx.arc(e.x-8,e.y-40,4,0,Math.PI*2);ctx.arc(e.x+8,e.y-40,4,0,Math.PI*2);ctx.fill()}
...
function forestAncient(e){ctx.fillStyle='#166534';ctx.strokeStyle='#14532d';ctx.lineWidth=4;rr(e.x-34,e.y-92,68,88,18);ctx.fill();ctx.stroke();ctx.fillStyle='#22c55e';ctx.beginPath();ctx.arc(e.x-24,e.y-96,20,0,Math.PI*2);ctx.arc(e.x+8,e.y-110,24,0,Math.PI*2);ctx.arc(e.x+30,e.y-88,18,0,Math.PI*2);ctx.fill();legs(e,'#78350f',8,-6,28)}
function frostGiant(e){ctx.fillStyle='#bae6fd';ctx.strokeStyle='#0369a1';ctx.lineWidth=4;rr(e.x-36,e.y-96,72,90,16);ctx.fill();ctx.stroke();legs(e,'#075985',8,-6,30);ctx.fillStyle='#e0f2fe';poly([[e.x-25,e.y-98],[e.x-10,e.y-128],[e.x,e.y-98],[e.x+12,e.y-128],[e.x+26,e.y-98]]);ctx.fill()}
function fireDemon(e){ctx.fillStyle='#991b1b';ctx.strokeStyle='#fb923c';ctx.lineWidth=4;rr(e.x-34,e.y-92,68,86,16);ctx.fill();ctx.stroke();legs(e,'#7f1d1d',8,-6,28);ctx.fillStyle='#fbbf24';poly([[e.x-28,e.y-94],[e.x-16,e.y-126],[e.x-4,e.y-94],[e.x+10,e.y-128],[e.x+28,e.y-94]]);ctx.fill();ctx.strokeStyle='#fb923c';ctx.lineWidth=5;line(e.x+34,e.y-58,e.x+54,e.y-78)}
...
function updateUI(){ui.gold.textContent=Math.floor(gold);ui.hp.textContent=Math.max(0,Math.floor(castleHp));ui.wave.textContent=wave;ui.map.textContent=`${MAP_CONFIG[currentMap].icon} ${MAP_CONFIG[currentMap].name} · ${DIFFICULTY_CONFIG[currentDifficulty].name}`;ui.kills.textContent=kills;ui.score.textContent=score;ui.mine.textContent=`${mineCost()} gold`;ui.walls.textContent=`${wallsCost()} gold`;ui.smith.textContent=`${smithCost()} gold`;updateUnitUpgradeUI();updatePurchaseButtons();if(score>best){best=score;GameStorage.setBestScore(best);ui.best.textContent=best}}
function unlockAchievement(id,notify=true){if(!id||!ACHIEVEMENT_CONFIG[id]||profile.achievements.includes(id))return;profile.achievements.push(id);GameStorage.saveProfile(profile);if(notify)text(W/2,145,`ACHIEVEMENT: ${ACHIEVEMENT_CONFIG[id].name}`,'#facc15')}
function showProfile(){profile=GameStorage.loadProfile();if(running)paused=true;let mapRows=Object.entries(MAP_CONFIG).map(([id,map])=>`<li>${map.icon} ${map.name}: ${profile.victories[id]?'completed':'not completed'}</li>`).join('');let achievements=Object.entries(ACHIEVEMENT_CONFIG).map(([id,a])=>`<li class="${profile.achievements.includes(id)?'done':'locked'}">${profile.achievements.includes(id)?'✅':'⬜'} ${a.name}<span>${a.description}</span></li>`).join('');ui.overlay.innerHTML=`<div class="panel profile-panel"><h2>🏆 Player Profile</h2><div class="profile-grid"><div><h3>Regions</h3><ul>${mapRows}</ul></div><div><h3>Achievements</h3><ul>${achievements}</ul></div></div><button id="overlayBtn">${running?'Continue':'Back'}</button></div>`;ui.overlay.style.display='grid';el('overlayBtn').onclick=()=>{if(running){paused=false;ui.overlay.style.display='none'}else showStartMenu()}}
function gameOver(){running=false;GameStorage.clearGame();cancelAnimationFrame(anim);showOverlay('💀 Castle Destroyed',`Final score: ${score}. You survived until wave ${wave} in ${MAP_CONFIG[currentMap].name}.`,'Campaign Menu',showStartMenu)}
...
window.startGame=startGame;window.continueGame=continueGame;window.spawnUnit=spawnUnit;window.upgradeUnitType=upgradeUnitType;window.buyUpgrade=buyUpgrade;window.saveGameManually=saveGameManually;window.togglePause=togglePause;window.showProfile=showProfile;
