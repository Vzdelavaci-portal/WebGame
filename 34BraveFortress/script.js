...
const W=960,H=520,groundY=410,castleX=85,spawnX=W+40;
...
const unitsCfg={
 swordsman:{cost:10,hp:100,damage:10,range:34,speed:1.25,rate:45,color:'#fbbf24'},
 archer:{cost:15,hp:65,damage:8,range:170,speed:1,rate:58,color:'#22c55e',ranged:true},
 knight:{cost:30,hp:240,damage:20,range:38,speed:.85,rate:60,color:'#38bdf8',shield:true},
 crossbowman:{cost:25,hp:80,damage:25,range:220,speed:.85,rate:82,color:'#a16207',ranged:true,bolt:true},
 eliteKnight:{cost:50,hp:450,damage:32,range:42,speed:.62,rate:72,color:'#facc15',shield:true,elite:true}
};
const enemiesCfg={
 goblin:{hp:55,damage:7,range:30,speed:.95,rate:52,reward:8,color:'#16a34a'},
 orc:{hp:125,damage:13,range:34,speed:.72,rate:62,reward:14,color:'#ea580c'},
 troll:{hp:260,damage:22,range:38,speed:.46,rate:75,reward:26,color:'#7c3aed'},
 skeleton:{hp:35,damage:5,range:28,speed:1.05,rate:55,reward:5,color:'#e5e7eb'},
 necromancer:{hp:160,damage:9,range:130,speed:.52,rate:95,reward:35,color:'#111827',summons:true},
 boss:{hp:1000,damage:34,range:48,speed:.38,rate:82,reward:120,color:'#dc2626',boss:true},
 dragon:{hp:1650,damage:46,range:150,speed:.55,rate:105,reward:220,color:'#b91c1c',boss:true,dragon:true,flying:true}
};
let units=[],enemies=[],projectiles=[],particles=[],texts=[];
let gold=100,castleHp=500,castleMaxHp=500,wave=1,kills=0,score=0,best=localStorage.getItem('braveFortressBestV2')||0;
let goldIncome=1,damageBonus=0,wallLvl=0,mineLvl=0,smithLvl=0,running=false,paused=false,anim,spawnTimer=0,spawned=0,toSpawn=0,waveActive=false,waveDelay=140,goldTimer=0,shake=0,frame=0;
ui.best.textContent=best;
function startGame(){units=[];enemies=[];projectiles=[];particles=[];texts=[];gold=100;castleHp=500;castleMaxHp=500;wave=1;kills=0;score=0;goldIncome=1;damageBonus=0;wallLvl=0;mineLvl=0;smithLvl=0;spawnTimer=0;spawned=0;toSpawn=countWave();waveActive=true;waveDelay=140;goldTimer=0;shake=0;frame=0;running=true;paused=false;ui.overlay.style.display='none';updateUI();cancelAnimationFrame(anim);loop()}
...
function countWave(){return wave%10===0?1:wave%5===0?1:5+wave*2}
function waves(){if(!waveActive){if(--waveDelay<=0){wave++;toSpawn=countWave();spawned=0;waveActive=true;text(W/2,82,wave%10===0?`DRAGON WAVE ${wave}`:wave%5===0?`BOSS WAVE ${wave}`:`WAVE ${wave}`,wave%10===0?'#b91c1c':wave%5===0?'#dc2626':'#fbbf24')}return}if(++spawnTimer>Math.max(34,95-wave*4)&&spawned<toSpawn){spawnForWave();spawned++;spawnTimer=0}if(spawned>=toSpawn&&enemies.length===0){waveActive=false;waveDelay=160;gold+=35+wave*5;text(W/2,82,`WAVE ${wave} CLEARED`,'#22c55e')}}
function spawnForWave(){if(wave%10===0)return spawnEnemy('dragon');if(wave%5===0)return spawnEnemy('boss');let r=Math.random();if(wave>=7&&r>.82)spawnEnemy('necromancer');else if(wave>=4&&r>.78)spawnEnemy('troll');else if(wave>=2&&r>.52)spawnEnemy('orc');else spawnEnemy('goblin')}
...
function spawnUnit(type){if(!running||paused)return;let c=unitsCfg[type];if(gold<c.cost){text(170,120,'Not enough gold','#dc2626');return}gold-=c.cost;units.push({type,x:castleX+55,y:groundY,hp:c.hp,maxHp:c.hp,damage:c.damage+damageBonus,range:c.range,speed:c.speed,rate:c.rate,cool:0,color:c.color,ranged:!!c.ranged,bolt:!!c.bolt,shield:!!c.shield,elite:!!c.elite});part(castleX+55,groundY,c.color,16)}
function updateUnits(){units.forEach(u=>{if(u.hp<=0){u.dead=true;part(u.x,u.y,u.color,18);return}let t=nearestEnemy(u);if(t&&Math.abs(t.x-u.x)<=u.range){if(--u.cool<=0){attack(u,t);u.cool=u.rate}}else u.x+=u.speed;u.x=Math.min(u.x,W-60)});units=units.filter(u=>!u.dead)}
function updateEnemies(){enemies.forEach(e=>{if(e.hp<=0){e.dead=true;kills++;gold+=e.reward;score+=e.boss?500:50;text(e.x,e.y-44,`+${e.reward} gold`,'#fbbf24');part(e.x,e.y,e.color,e.boss?50:22);return}if(e.summons&&--e.summonTimer<=0){spawnSkeleton(e.x+35);e.summonTimer=300;text(e.x,e.y-55,'SUMMON','#e5e7eb')}let t=nearestUnit(e);if(t&&Math.abs(t.x-e.x)<=e.range){if(--e.cool<=0){e.dragon?dragonFire(e):hitUnit(e,t);e.cool=e.rate}}else if(e.x<=castleX+45){if(--e.cool<=0){hitCastle(e.damage);e.cool=e.rate}}else e.x-=e.speed});enemies=enemies.filter(e=>!e.dead)}
...
function attack(u,e){let crit=Math.random()<.1,dmg=crit?u.damage*2:u.damage;if(crit)text(e.x,e.y-70,'CRITICAL!','#facc15');if(u.ranged)projectiles.push({x:u.x+10,y:u.y-26,target:e,color:u.color,damage:dmg,life:120,bolt:u.bolt});else{e.hp-=dmg;part(e.x,e.y-18,u.color,crit?18:8)}}
function hitUnit(e,u){u.hp-=e.damage;part(u.x,u.y-20,e.color,8)}
function hitCastle(d){castleHp-=d;shake=8;part(castleX,groundY-70,'#dc2626',20);if(castleHp<=0)gameOver()}
function dragonFire(e){units.forEach(u=>{if(Math.abs(u.x-e.x)<=e.range){u.hp-=e.damage;part(u.x,u.y-28,'#f97316',18)}});for(let i=0;i<30;i++)particles.push({x:e.x-42,y:e.y+8,vx:-Math.random()*5,vy:(Math.random()-.5)*3,life:32,color:Math.random()>.5?'#f97316':'#facc15'})}
function updateProjectiles(){projectiles.forEach(p=>{let e=p.target;if(!e||e.hp<=0){p.dead=true;return}let dx=e.x-p.x,dy=e.y-22-p.y,dist=Math.hypot(dx,dy);if(dist<8){e.hp-=p.damage;p.dead=true;part(e.x,e.y-18,p.color,p.bolt?18:10)}else{p.x+=dx/dist*(p.bolt?10:8);p.y+=dy/dist*(p.bolt?10:8)}p.life--});projectiles=projectiles.filter(p=>!p.dead&&p.life>0)}
...
function drawProj(p){ctx.fillStyle=p.color;ctx.strokeStyle=p.color;ctx.lineWidth=p.bolt?3:1;if(p.bolt){line(p.x-8,p.y,p.x+8,p.y)}else{ctx.beginPath();ctx.arc(p.x,p.y,4,0,Math.PI*2);ctx.fill()}}
...
function updateUI(){ui.gold.textContent=Math.floor(gold);ui.hp.textContent=Math.max(0,Math.floor(castleHp));ui.wave.textContent=wave;ui.kills.textContent=kills;ui.score.textContent=score;ui.mine.textContent=`${mineCost()} gold`;ui.walls.textContent=`${wallsCost()} gold`;ui.smith.textContent=`${smithCost()} gold`;if(score>best){best=score;localStorage.setItem('braveFortressBestV2',best);ui.best.textContent=best}}
function gameOver(){running=false;cancelAnimationFrame(anim);showOverlay('💀 Castle Destroyed',`Final score: ${score}. You survived until wave ${wave}.`,'Play Again',startGame)}
...
window.startGame=startGame;window.spawnUnit=spawnUnit;window.buyUpgrade=buyUpgrade;
bg();buildings();castle();updateUI();
